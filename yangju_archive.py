...
with tabs[1]:
    ...
    try:
        ...
        if df_yg.empty:
            st.warning("❗ '양주시' 데이터가 존재하지 않습니다.")
        else:
            # 컬럼에서 2005년 이상, 5년 단위만 뽑기
            years = []
            births = []
            deaths = []
            for col in df_yg.columns:
                if "출생건수" in col:
                    ...
                    if (year >= 2005 and year % 5 == 0) or (year == df_yg.columns[-2].split()[0]) or (year == df_yg.columns[-1].split()[0]):
                        years.append(year)
                        births.append(int(df_yg[col].values[0]))
            for col in df_yg.columns:
                if "사망건수" in col:
                    year = int(col.split()[0])
                    if (year >= 2005 and year % 5 == 0) or (year == df_yg.columns[-2].split()[0]) or (year == df_yg.columns[-1].split()[0]):
                        deaths.append(int(df_yg[col].values[0]))
            # 마지막 연도가 5년 단위에 포함되지 않으면, 마지막 연도도 강제로 추가
            if len(births) != len(deaths):
                minlen = min(len(births), len(deaths))
                births, deaths = births[:minlen], deaths[:minlen]
            # 시각화
            fig, ax = plt.subplots(figsize=(7, 4))
            ax.plot(years, births, marker='o', label='출생자수')
            ax.plot(years, deaths, marker='o', label='사망자수')
            ax.set_title("양주시 5년 단위 출생자수·사망자수 변화", fontproperties=font_prop, fontsize=15)
            ax.set_xlabel("연도", fontproperties=font_prop, fontsize=11)
            ax.set_ylabel("명", fontproperties=font_prop, fontsize=11)
            ax.set_xticks(years)
            ax.set_xticklabels(years, fontproperties=font_prop, fontsize=10)
            ax.legend(prop=font_prop, fontsize=11)
            plt.yticks(fontproperties=font_prop, fontsize=10)
            plt.tight_layout()
            st.pyplot(fig)
            st.caption("양주시 인구 구조 변화는 5년 단위로 간략하게 시각화합니다. 데이터 출처: KOSIS 국가통계포털")
