...
# 1. 웹사이트 본문 폰트 크기 일괄 적용 (16px 기본, 18px 타이틀 등)
st.markdown("""
    <style>
    html, body, [class*="css"]  {
...
# 3. Streamlit 페이지 설정
st.set_page_config(page_title="양주시 아카이브: 과거, 현재, 미래, 인구 변화", layout="wide")
st.title("🏙️ 양주시 아카이브: 과거, 현재, 미래, 인구 변화")
st.markdown("<span style='font-size:15pt;'>경기도 양주시의 역사와 미래 비전과 인구 변화를 살펴보는 디지털 아카이브입니다.</span>", unsafe_allow_html=True)
...
with tabs[3]:
    ...
    # --- 인구 변화 그래프 (월별/연도별)
    POP_PATH = "양주시_연도별_인구수.csv"
    try:
        df_pop = pd.read_csv(POP_PATH, encoding="cp949")
        df_pop_yg = df_pop[df_pop['행정구역별(시군구)'] == "양주시"]
        
        if '기간' not in df_pop_yg.columns or '총인구수(명)' not in df_pop_yg.columns:
            st.warning("인구수 데이터에 '기간' 또는 '총인구수(명)' 컬럼이 없습니다. 컬럼명을 확인해주세요.")
        else:
            years_pop = df_pop_yg['기간'].tolist()
            pop_counts = df_pop_yg['총인구수(명)'].tolist()
            
            fig_pop, ax_pop = plt.subplots(figsize=(10, 3))
            ax_pop.plot(years_pop, pop_counts, marker='o', color='green', label='총 인구수')
            ax_pop.set_title("양주시 월별·연도별 인구수 변화", fontproperties=font_prop, fontsize=13)
            ax_pop.set_xlabel("기간", fontproperties=font_prop, fontsize=11)
            ax_pop.set_ylabel("명", fontproperties=font_prop, fontsize=11)
            ax_pop.tick_params(axis='x', rotation=45)
            ax_pop.legend(prop=font_prop, fontsize=10)
            plt.tight_layout()
            st.pyplot(fig_pop)
    ...
    # --- 출생자수/사망자수 그래프 (5년 단위)
    BIRTH_DEATH_PATH = "양주시_연도별_출생자수_사망자수.csv"
    try:
        df_bd = pd.read_csv(BIRTH_DEATH_PATH, encoding="cp949")
        df_bd['행정구역별'] = df_bd['행정구역별'].astype(str).str.strip()
        df_bd_yg = df_bd[df_bd['행정구역별'] == "양주시"]
        df_bd_yg = df_bd_yg.reset_index(drop=True)

        colnames = list(df_bd_yg.columns)
        ...
        for col in birth_cols:
            ...
            if year_match:
                ...
                if y >= 2005 and (y % 5 == 0 or y == int(birth_cols[-1][:4])):
                    ...
                    try:
                        val = int(str(df_bd_yg.iloc[0][col]).replace(",", "").strip())
                    except:
                        val = 0
                    births.append(val)
        ...
        for col in death_cols:
            ...
            if year_match:
                ...
                if y >= 2005 and (y % 5 == 0 or y == int(death_cols[-1][:4])):
                    ...
                    try:
                        val = int(float(str(df_bd_yg.iloc[0][col]).replace(",", "").strip()))
                    except:
                        val = 0
                    deaths.append(val)
        ...
        fig_bd, ax_bd = plt.subplots(figsize=(10, 3))
        ax_bd.plot(common_years, births_aligned, marker='o', color='tab:blue', label='출생자수')
        ax_bd.plot(common_years, deaths_aligned, marker='o', color='tab:orange', label='사망자수')
        ax_bd.set_title("양주시 5년 단위 출생자수·사망자수 변화", fontproperties=font_prop, fontsize=13)
        ax_bd.set_xlabel("연도", fontproperties=font_prop, fontsize=11)
        ax_bd.set_ylabel("명", fontproperties=font_prop, fontsize=11)
        ax_bd.set_xticks(common_years)
        ax_bd.set_xticklabels(common_years, fontproperties=font_prop, fontsize=9)
        plt.yticks(fontproperties=font_prop, fontsize=9)
        plt.xticks(fontproperties=font_prop, fontsize=9)
        ax_bd.legend(prop=font_prop, fontsize=10)
        plt.tight_layout()
        st.pyplot(fig_bd)
        st.caption("양주시 인구 구조 변화를 5년 단위로 시각화. 데이터 출처: KOSIS 국가통계포털")
    except Exception as e:
        st.warning(f"출생자수/사망자수 그래프 로드 중 오류가 발생했습니다: {e}")
